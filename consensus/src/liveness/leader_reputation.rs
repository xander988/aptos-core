...
pub fn extract_epoch_to_proposers(
    ...
    first_epoch_to_consider: u64,
    needed_rounds: u64,
) -> Result<HashMap<u64, Vec<Author>>> {
    ensure!(proof.ledger_info_with_sigs.len() as u64 == (epoch - (first_epoch_to_consider - 1)));
    let last_index = proof.ledger_info_with_sigs.len() - 1;
    let mut num_rounds = 0;
    let mut result = HashMap::new();
    for (index, ledger_info) in proof.ledger_info_with_sigs.into_iter().enumerate().rev() {
        let cur_epoch_state = ledger_info
            .ledger_info()
            .next_epoch_state()
        ...
        if index == last_index {
                ...
                epoch == cur_epoch_state.epoch,
                "fetched epoch_ending ledger_infos are for a wrong epoch {} vs {}",
                epoch,
                cur_epoch_state.epoch
            );
            ensure!(
                proposers == &cur_epoch_proposers,
                "proposers from state and fetched epoch_ending ledger_infos are missaligned"
            );
        }
        result.insert(cur_epoch_state.epoch, cur_epoch_proposers);
        ...
        num_rounds += ledger_info.ledger_info().round();
    }
